...
## Third remote review fixes: mutation ban and empty filters

Review of `5542da4` reported two P2 findings: mutation authorization did not recheck a ban committed after authentication (`PRRT_kwDOQf7Ojc6jtYCE`), and empty subscriber filter values were silently ignored (`PRRT_kwDOQf7Ojc6jtYCJ`). Both regression tests failed before the fixes (successful mutation and HTTP 200 respectively). Mutation authorization now checks the same current-ban predicate under its existing locks. Supplied filters are validated whenever non-null, including empty strings; the same adjacent bug in media kind/page filters was corrected. Regression checks all four filters return HTTP 400 without running a listing query.

Validation: 260 tests / 35 files, TypeScript, full lint (unchanged five baseline warnings), production build and whitespace checks pass. Next action: push, resolve both supported findings, request fresh review, verify current-head remote checks and review.
